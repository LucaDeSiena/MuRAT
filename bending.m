% FUNCTION BENDING: Bends the initial segment along the normal to the ray
% path tangent at each point by an optimal distance r.
function [xtemp,ytemp,ztemp,v]=bending(xtemp,ytemp,ztemp,gridD,v,pvel)
    xfac=1;
    n = length(xtemp);
    for k=2:(n-1)
        kk=k-1;
        ...
        vk=velocity(xk,yk,zk,gridD,pvel);
        [vx,vy,vz]=veldicity(xk,yk,zk,gridD,pvel);
        ...
        if (rvs == 0)
            ...
            ytemp(k)=yyk;
            ztemp(k)=zzk;
            vk = velocity(xxk,yyk,zzk,gridD,pvel);
            v(k)=vk;
        end
    end
end
