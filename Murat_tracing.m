...
function ma         =   Murat_tracing(ray,gridD,pvel)

maxit               =   100; %Maximum number of iterations allowed
maxpoints           =   10000; %Max number points to bend in one direction
max3                =   3*maxpoints; %Three dimensions

dconv=sqrt((gridD(1,2)-gridD(1,1))^2 + (gridD(2,2)-gridD(2,1))^2 +...
    (gridD(3,2)-gridD(3,1))^2);
tlim=dconv/mean(mean(mean(pvel*1000))); %Time limit
...
for i=1:n
    ...
    yp=ytemp(i);
    zp=ztemp(i);
    vp=Murat_velocity(xp,yp,zp,gridD,pvel);
    v(i,1)=vp;
end

[ta,tra]=Murat_traveling(xtemp,ytemp,ztemp,gridD,v,pvel);
...
ma(1:n,1:5) = [(1:n)',xtemp',ytemp',ztemp',ta'/1000]; %Final ray
end