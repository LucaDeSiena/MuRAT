...
if evst==2
    ...
    if degorutm==111
        ...
        xlim([XY(1,1) XY(end,1)]);ylim([XY(1,2) XY(end,2)]);
    end
    if pa<3
...
if degorutm==111
    hold on
    geoshow(coastlat,coastlon);
    xlim([originWE originWE+nxc*stepg]);ylim([originSN originSN+nyc*stepg]);
end
hold off
...
elseif pa>=2
    ...
    Qcchecko=figure('Name','Qc checkerboard test input','NumberTitle',...
        'off','visible',visib);
    contourf(X,Y,QQcho');
    axis equal
    view(2)
...
if pa==3
    ...
    V_map=figure('Name','Velocity Model','NumberTitle','off','visible',visib);
    ...
    Q_map=figure('Name','Velocity Model','NumberTitle','off','visible',visib);
