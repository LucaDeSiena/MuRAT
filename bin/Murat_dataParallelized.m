...
% MODIFICATION:
% set a default envelope smoothing time if not defined in the input
...
% MODIFICATION:
% use a logical mask to mark discarded traces inside the parfor loop.
% This is more robust in parallel execution than incrementing a shared counter.
trashMask           =   false(nData,1);
...
parfor (i = 1:nData,workers)
    ...
    % MODIFICATION:
    % pass the envelope smoothing parameter defined in the input
    ...
    % Reject traces that do not meet coda/travel requirements
    ...
    if doKernels
        % Compute and plot kernels and coda matrix
        TCodaStart                  =   tCoda_i;
