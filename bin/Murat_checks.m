...
FLabel              =   s.label;
FPath               =   './';
velocityModel       =   fullfile('velocity_models', s.namev);

% time fields
PTime               =   ['SAChdr.times.' s.PTime];
PorS                =   s.POrS;
origin              =   s.origin;
ending              =   s.end;
nLat                =   s.gridLat;
nLong               =   s.gridLong;
nzc                 =   s.gridZ;
availableVelocity   =   s.availableVelocity;

if isempty(s.originTime)
    originTime      =   'SAChdr.times.o';
else
    originTime      =   ['SAChdr.times.' s.originTime];
end

if isempty(s.STime)
    STime           =   'SAChdr.times.t0';
else
    STime           =   ['SAChdr.times.' s.STime];
end
...
[d,az]      =   distance(origin(1),origin(2),ending(1),ending(2),wgs84);
...
if availableVelocity ==  0
    ...
    if isequal(s.namev,"LITHO1.0.nc")
...
outDirTXT               =   fullfile(FLabel,'Tests','DataHeaders.xlsx');
