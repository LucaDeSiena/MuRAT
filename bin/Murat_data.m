...
% MODIFICATION:
% Set a default envelope smoothing time if it is not defined in the input.
% This keeps backward compatibility with old input files that do not include
% the envelopeSmoothTime field.
if ~isfield(s,'envelopeSmoothTime') || isempty(s.envelopeSmoothTime)
    s.envelopeSmoothTime = 1.0;
end
...
% Count waveforms removed because they do not satisfy coda/travel constraints
countTrash          =   0;

%==========================================================================
% Extract SAC headers once before the main loop to avoid repeated dynamic
% field access inside the waveform-processing loop.
...
for i = 1:nData
    ...
    % MODIFICATION:
    % Pass the envelope smoothing parameter explicitly.
    [tempis,sp_i]   =   Murat_envelope(cf,listSac_i,s.envelopeSmoothTime);

    % Locations (degrees and meters)
    [locationDeg_i, locationM_i]    =   Murat_location(origin,SAChdr_i);
    locationDeg(i,:)                =   locationDeg_i;

    % Check direct-wave picking
    ...
    % Decide whether ray-dependent computations are required for this component
    doRays                          =   recognizeComponents(i, components);

    if doRays
        % Ray-dependent parameters
    ...
    % Reject traces that do not satisfy coda/travel requirements
    ...
    % Measure Qc and its uncertainty
    [inverseQc_i, uncertaintyQc_i]  =   Murat_Qc(cf,...
        sp_i,cursorCodaStart_i,cursorCodaEnd_i,tCoda_i,srate_i,QcMeasure);

    % Decide whether the kernel/inversion matrix for Qc is needed
    doKernels = doRays;

    if doKernels
        % Compute kernels and coda matrix
        TCodaStart = tCoda_i;
        TCodaEnd   = tCoda_i + tCodaWindow;

        [KgridS, rgridS] = Murat_kernels(...
            TCodaStart,locationM_i(1:3),locationM_i(4:6),...
            modvQc,vS,kernelThresh,B0,Le,lapseTimeMethod);

        [KgridE, rgridE] = Murat_kernels(...
            TCodaEnd,locationM_i(1:3),locationM_i(4:6),...
            modvQc,vS,kernelThresh,B0,Le,lapseTimeMethod);
...
% Assign outputs to the Murat structure
...
% LOGICAL to decide if forward modelling is necessary depending on waveform
